...
<script>
    function showConfirmModal(action, appointmentId, patientName) {
        ...
        // Remove any existing hidden status input and add new one
    ...
    function showStatusModal(appointmentId) {
        const select = document.querySelector(`#appointment-${appointmentId} select`) ||
                       document.querySelector(`form[action*="/${appointmentId}/status"] select`);
